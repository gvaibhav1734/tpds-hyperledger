...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/stategovernmentdepot.example.com/connection-stategovernmentdepot.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/stategovernmentdepot.example.com/connection-stategovernmentdepot.yaml
...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/statelevelfps.example.com/connection-statelevelfps.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/statelevelfps.example.com/connection-statelevelfps.yaml
