...
export default function Home() {
    ...
    const sendAsset = async event => {
        ...
        var sendreq = JSON.stringify({
            ID: event.target.id.value,
            From: event.target.from.value,
            To: event.target.to.value
        });
        console.log(`sendreq: ${sendreq}`);
        const res = await fetch('/api/sendAsset', {
            body: sendreq,
            headers: {
        ...
        if (txn_response == null) {
            txn_response = "Send Transaction Failed";
        }
      ...
      <main>
        ...
        <div className="grid">
            <form onSubmit={sendAsset}>
                ...
                <button type="submit" className="button button1" >Send</button>
            </form>
        </div>
