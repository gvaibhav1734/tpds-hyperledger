...
exports.buildCCPStategovernmentfps = () => {
	// load the common connection configuration file
	const ccpPath = path.resolve(__dirname, '..', 'organizations', 'peerOrganizations', 'stategovernmentfps.example.com', 'connection-stategovernmentfps.json');
	const fileExists = fs.existsSync(ccpPath);
	if (!fileExists) {
		throw new Error(`no such file or directory: ${ccpPath}`);
	}
